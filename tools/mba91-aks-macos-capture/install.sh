...
echo "  1) Enable private data BEFORE reboot:"
...
echo "  2) Reboot once (cold-boot capture)."
echo "  3) Log in, enroll Touch ID while capture is running."
echo "  4) Copy newest files from $LOG_DIR off-machine."
echo "  5) sudo ./uninstall.sh  (+ remove the Device Management profile)"
echo
