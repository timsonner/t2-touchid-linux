...
"""DRAFT (staged, not yet run): E4 full enroll dance, ring finger.

Status: design only. LIVE_E4_ENABLED defaults False; even enabled,
this refuses unless ALL of the following hold:

- warm SEP: BridgeXPC 0x42 holds EXACTLY 2 identities for the
  configured UID (E4 expects the 2->3 transition; SKS informational);
...
def fail(message: str) -> int:
    print(f"enroll-e4: {message}", file=sys.stderr)
    return 1


def e4_consumer_factory(host, port, interface, user_id, post_4_standin):
    def consume(external_form: bytes) -> dict[str, object]:
        ...
        try:
            ...
            if count != 2:
                raise RuntimeError(
                    "E4 expects exactly 2 identities (2->3); refusing")
            ...
            if after == 3:
                summary["outcome"] = "enrolled-pending-reboot-proof"
            elif after == 2:
...
def main() -> int:
    ...
    if not LIVE_E4_ENABLED or args.confirm_live != CONFIRM:
        return fail("live E4 probe is disabled in source; refusing")
    ...
    try:
        with ACMDevice() as device:
            initial, final, e4_summary = with_authorized_context(
                device, user_id, bind_password,
                e4_consumer_factory(
                    args.host, args.port, args.interface, user_id,
                    args.with_post_4_standin),
                tracking=True)
    ...
    if e4_summary.get("outcome") == "ambiguous-halt-everything":
        return fail("E4 ambiguous state; halt all live work and reconcile")
    return 0
